...
**What's New**
- **swidge-symbiosis** ([v1.1.1](https://github.com/symbiosis-finance/wdk-protocol-swidge-symbiosis/tree/v1.1.1)): Add the community-maintained Symbiosis Swidge provider for dynamic chain and token discovery, indicative cross-chain quotes, source-chain approval and execution, and status polling. Execution obtains a fresh route rather than consuming the earlier quote, so applications must reconfirm the current response and enforce their own output policy before broadcast.
