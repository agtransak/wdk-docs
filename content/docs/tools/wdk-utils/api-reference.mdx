...
Validate an address with the chain-specific validator selected by a CAIP-2 chain ID. The supported namespaces are `bip122`, `eip155`, `solana`, `spark`, and `tron`.
...
- `INVALID_CHAIN_ID`: `chainId` is not a string or does not match the accepted CAIP-2 shape: a 3–8 character lowercase namespace with an optional 1–32 character reference.
