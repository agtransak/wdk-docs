...
- **Gasless route defaults**: Filters native-value bridge routes by default so ERC-4337 sponsored execution can cover gas without the LI.FI route requiring separate native token value.
...
| No account | Chain and token discovery only. |

Use a read-only EVM account when you need quotes without signing. LI.FI quotes require a source address, so no-account mode is limited to chain and token discovery.
