...
`account.shutdown()` is idempotent, but `manager.dispose()` is the terminal owner-level cleanup. It shuts down the binding, destroys the VLS signer, wipes retained seed buffers, and invalidates read-only adapters. `account.dispose()` is a no-op.

Do not reuse a manager, account, read-only adapter, or LSP object after manager disposal.
