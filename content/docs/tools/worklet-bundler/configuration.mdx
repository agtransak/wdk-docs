...
npm install --save-dev @tetherto/wdk-worklet-bundler@1.0.0-beta.6
```

Worklet Bundler beta.6 generates entrypoints that import Pear Worklet, but `generate --install` does not add that package. Pear Worklet beta.10 provides the JSON-RPC, HRPC generic-module, and Swidge protocol behavior documented here.
...
<Callout type="warn">
Beta.6 generic modules are included only in generated HRPC entrypoints. Do not configure `modules` with `transport: 'jsonrpc'`; the current schema accepts the field, but JSON-RPC generation ignores it.
</Callout>
...
- `types`: Declared in the beta.6 config type, but the generator does not honor a custom value and always writes declarations to `./.wdk/index.d.ts`.
...
- `minify` (`boolean`): Declared in the beta.6 config type but not read by the bundle path. JSON-RPC ESM-to-CJS conversion minifies independently of this field.
- `sourceMaps` (`boolean`): Declared in the beta.6 config type but not read by the bundle path, so it does not produce source maps.
...
Worklet Bundler beta.6 separates required peers from peers explicitly marked optional through `peerDependenciesMeta`:
...
- Installed optional peers are bundled normally. A peer that is required anywhere else in the scanned dependency tree is treated as required and is not deferred.
