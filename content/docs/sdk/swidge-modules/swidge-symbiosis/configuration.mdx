...
|---|---|
| `account` | Optional WDK wallet account. Discovery and some quotes can run without one; execution requires the capabilities used by the returned route. |
...
| `maxNetworkFeeBps` | `number \| bigint` | None | Shared network-fee cap. The current provider fee mapping does not emit a `network` fee item, so this does not constrain a separate network fee in `1.1.1`. |
| `maxProtocolFeeBps` | `number \| bigint` | None | Rejects execution when mapped Symbiosis protocol fees exceed this many basis points of the input amount. |
...
For TON and Solana assets, the module returns the provider's native-format address from token discovery when available.
...
The module converts the decimal value to basis points for the Symbiosis request. It does not separately validate an additional `minAmountOut` field if a newer WDK interface exposes one.

## Refund address

Set a refund address suitable for the source route:
...
```javascript
await symbiosis.swidge({
  fromToken: 'BTC',
  toToken: 'USDC',
  toChain: 'Arbitrum One',
  recipient: '0xRecipient...',
  refundAddress: 'bc1qDifferentRefund...',
  fromTokenAmount: 50_000n
})
```

The module forwards the address to Symbiosis; it does not validate that the address belongs to the intended refund chain.
...
When both the input token and fee token have USD price data, the module compares USD values. Otherwise it compares decimal-normalized token amounts. That fallback is approximate when the fee token and input token have different unit values.

Symbiosis fee entries currently map to `protocol` or `affiliate`. No returned entry maps to `network`, so `maxNetworkFeeBps` remains at zero for this provider's current fee calculation.
