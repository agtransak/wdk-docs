...
**What's New**
- **wdk-wallet** ([v1.0.0-beta.15](https://github.com/tetherto/wdk-wallet/releases/tag/v1.0.0-beta.15)): Add the provider-facing `SdaProtocol` and `ISdaProtocol` interface for supported-route discovery and deposit-address creation, with optional quoting, address derivation and lifecycle, transfer history and status, and recovery methods. No concrete SDA provider or WDK Core registration/accessor ships in the current releases, so this is an implementation contract rather than a runnable end-user flow. The package also exports `UnsupportedOperationError`, `ValueError`, and `NoSuchElementError`.
