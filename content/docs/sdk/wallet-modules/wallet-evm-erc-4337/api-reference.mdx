...
**Throws:** Error if a non-sponsored operation exceeds `transactionMaxFee`
...
The returned operation contains the current nonce and the fee-mode configuration used while building it. Submit it promptly through the same account and do not mutate it.
</Callout>
...
**Throws:** Error if a non-sponsored, newly built operation exceeds `transactionMaxFee`
...
UserOperations built while quoting transaction inputs are cached internally for up to 2 minutes. When a later `sendTransaction()` or `signTransaction()` matches the quoted transaction, the account performs a lightweight on-chain nonce check before reuse and re-quotes if the nonce has moved. A supplied signed UserOperation is evaluated directly rather than rebuilt.
...
The quote cache does not include these override fields in its key. Pass the same fee-mode and paymaster settings to a quote and its matching send, sign, or transfer.
