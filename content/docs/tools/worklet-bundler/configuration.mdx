...
- Installed optional peers are bundled normally. Beta.7 searches root, nested, scoped, and symlinked package trees before classifying a peer as missing; this prevents an installed nested dependency from being deferred and then failing with `MODULE_NOT_FOUND` at runtime. A peer that is required anywhere else in the scanned dependency tree is treated as required and is not deferred.
