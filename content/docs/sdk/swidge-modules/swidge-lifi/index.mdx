...
- **Quotes and discovery without signing**: Supports route quotes plus chain and token discovery without a bound wallet account when a provider is configured.
...
| No account | Quotes plus chain and token discovery when `config.provider` is supplied. |
