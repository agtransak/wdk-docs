...
| `ramp.moonpay.apiKey` | MoonPay API key | Empty |
| `ramp.moonpay.signUrl` | URL of your MoonPay signing service | Empty |
...
Prefer `WDK_INDEXER_API_KEY` when you do not want to persist the indexer key. No environment override is available for the other supported secrets in beta.1.
...
Transfer history uses the WDK Indexer API:
...
Set a custom endpoint when needed:
...
`wdk buy` and `wdk sell` require all three MoonPay values:

```bash title="Terminal"
wdk config set --key ramp.moonpay.apiKey --value YOUR_MOONPAY_API_KEY
...
Use `sandbox` for testnets and `production` for mainnets. The CLI rejects a sandbox/mainnet or production/testnet mismatch.
