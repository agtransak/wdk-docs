...
`manager.dispose()` is terminal. It owns binding shutdown, VLS signer destruction, retained-seed cleanup, and account invalidation. `account.dispose()` is a no-op in this release.
